...
\begin{docKey}[anim]{frame}{=\meta{spec}}{no default}
  By default \cs{reveal} makes an element visible only during the \emph{current}
  step (the step in which it appears in the source).  The \texttt{frame} key
  ...
  \begin{itemize}
    \item Single step — \texttt{frame=2}
    \item Range — \texttt{frame=2-5} (steps 2, 3, 4, 5)
    \item List — \texttt{frame=\{1,3\}} (braces required when listing multiple values)
    \item Mix  — \texttt{frame=\{1,3-5\}} (braces required)
  \end{itemize}
