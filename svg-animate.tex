...
\begin{docKey}[anim]{blink off opacity}{=\meta{value}}{initial: \texttt{0}}
  Opacity used during the \emph{``off''} (hidden) half-periods of a blink.
  Defaults to \texttt{0} (fully hidden).  Independent of \docValue{inactive opacity},
  which controls the opacity \emph{outside} the active step.

  This distinction matters when \docValue{inactive opacity} is set above zero:
  the element can be \emph{dimmed} between steps while still
  \emph{fully hiding} during its blink-off phases.

  To suppress blinking in the ``all steps at opacity~1'' idiom, set
  \texttt{blink on opacity=1, blink off opacity=1} at the environment level.

\begin{dispListing}
%% Warning icon: dimmed at 0.5 between steps, blinks to fully hidden during step 2.
